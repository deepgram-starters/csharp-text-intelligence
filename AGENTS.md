# csharp-text-intelligence

C# (.NET) demo app for Deepgram Text Intelligence.
...
| File | Purpose |
|------|---------|
| `Program.cs` | Main backend — API endpoints and request handlers |
...
```
feat(csharp-text-intelligence): add diarization support
fix(csharp-text-intelligence): resolve WebSocket close handling
refactor(csharp-text-intelligence): simplify session endpoint
